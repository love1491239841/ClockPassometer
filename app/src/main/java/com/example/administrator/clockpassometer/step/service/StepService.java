package com.example.administrator.clockpassometer.step.service;
...
import com.example.administrator.clockpassometer.step.accelerometer.StepCount;
import com.example.administrator.clockpassometer.step.accelerometer.StepValuePassListener;
import com.orhanobut.logger.Logger;

import java.text.SimpleDateFormat;
import java.util.Date;
import java.util.List;
import com.example.administrator.clockpassometer.step.utils.*;
import com.example.administrator.clockpassometer.step.bean.*;
public class StepService extends Service implements SensorEventListener {
    ...
    private void initNotification() {
        ...
        mBuilder.setContentText("今日步数" + CURRENT_STEP + " 步")
                .setContentIntent(getDefalutIntent(Notification.FLAG_ONGOING_EVENT))
                .setWhen(System.currentTimeMillis())//通知产生的时间，会在通知信息里显示
                .setPriority(Notification.PRIORITY_DEFAULT)//设置该通知优先级
                .setAutoCancel(false)//设置这个标志当用户单击面板就可以让通知将自动取消
                .setOngoing(true)//ture，设置他为一个正在进行的通知。他们通常是用来表示一个后台任务,用户积极参与(如播放音乐)或以某种方式正在等待,因此占用设备(如一个文件下载,同步操作,主动网络连接)
                .setSmallIcon(R.mipmap.des);
